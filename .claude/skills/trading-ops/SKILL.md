...
- `GET /equity?secret=...` … **資産推移(NAV指数)**。約定履歴＋bitbank公開日足から戦略NAV（保有銘柄の等ウェイト日次リターンの連鎖＝入出金の影響を受けない実質リターン）を再構築しBTC買い持ちと比較。純資産額は非表示。今後分は `data/perf_log.csv` の `nav` 列が同じ指標を継ぎ足す。
